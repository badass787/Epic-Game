...
void game::upd_enemies() {
	...
	for (auto* ENEMY : this->enemies) {
		...
		//crashing with player part

		/*else if (ENEMY->getBounds_e().intersects(this->PLAYER->getBounds()) and ENEMY->getType() == 10) {
				delete this->enemies.at(counter);
				this->enemies.erase(this->enemies.begin() + counter);
				--counter;
				if (ENEMY->getTypeAGAIN() == 0) {
					this->PLAYER->increaseATT();
				}
				else if (ENEMY->getTypeAGAIN() == 1) {
					this->PLAYER->increaseSPID();
				}
				//this->PLAYER->loseHP(this->ENEMY->getDamage());
...
void game::ren()
{
	...
	//game over
	if (this->deadline.getElapsedTime().asSeconds() > 150) {
		if (points >=2500){
